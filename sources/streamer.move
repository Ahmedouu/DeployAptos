module the_streamer::streamer {
    ...
    public entry fun create_stream(
        ...
        let stream_counter = borrow_global_mut<StreamCounter>(@the_streamer);
        ...
        let receiver_lookup = borrow_global_mut<StreamReceiverLookup>(@the_streamer);
        ...
        event::emit_event(
            &mut borrow_global_mut<ModuleEventStore>(@the_streamer).stream_create_events,
            StreamCreateEvent {
    ...
    public entry fun accept_stream(receiver: &signer, sender_address: address) acquires StreamStore, ModuleEventStore {
        ...
        event::emit_event(
            &mut borrow_global_mut<ModuleEventStore>(@the_streamer).stream_accept_events,
            StreamAcceptEvent {
                sender_address,
                receiver_address,
                stream_id: stream.id,
    ...
    public entry fun claim_stream(receiver: &signer, sender_address: address) acquires StreamStore, StreamReceiverLookup, ModuleEventStore {
        ...
        if (coin::value(&coins) <= claim_amount) {
            ...
            event::emit_event(
                &mut borrow_global_mut<ModuleEventStore>(@the_streamer).stream_close_events,
                StreamCloseEvent {
                    sender_address,
                    receiver_address,
                    amount_to_receiver,
            ...
            event::emit_event(
                &mut borrow_global_mut<ModuleEventStore>(@the_streamer).stream_claim_events,
                StreamClaimEvent {
                    sender_address,
                    receiver_address,
                    amount: claim_amount,
    ...
    acquires StreamReceiverLookup, StreamStore { 
        let receiver_lookup = borrow_global<StreamReceiverLookup>(@the_streamer);
